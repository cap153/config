...
FONT_DIR="$HOME/.local/share/fonts/segoeui"
if [[ ! -d "$FONT_DIR" ]]; then
	echo "Installing Emoji fonts. If the download fails, you can try deleting the directory $FONT_DIR and restarting zsh."
	mkdir -p $FONT_DIR
	curl -L "https://github.com/cap153/config/releases/download/%E6%88%91%E4%BD%BF%E7%94%A8%E7%9A%84%E5%AD%97%E4%BD%93/segoeui.tar.gz" -o /tmp/font.tar.gz
	tar -zxvf /tmp/font.tar.gz -C $FONT_DIR
fi
